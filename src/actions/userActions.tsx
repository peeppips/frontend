...
export const getUserDetails = (uid:string) => async (dispatch: Dispatch<AnyAction>,getState: () => { userLogin: { userInfo: any; }; }) => {
  try {
    dispatch({
    ...
    const { data } = await axios.get(`https://peeppipsbackend.onrender.com/api/users/${uid}`,config)
    console.log(data)
    dispatch({
...
export const updateUserProfile = (uid:string,updatedData:any) => async (dispatch: Dispatch<AnyAction>, getState:any) => {
  try {
    dispatch({
    ...
    const { data } = await axios.put(`https://peeppipsbackend.onrender.com/api/users/${uid}`, updatedData, config)
